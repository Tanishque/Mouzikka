...
class view:
    ...
    def setup_player(self):
        try:
            ...
            if self.my_player.get_db_status():
                ...
                raise Exception("Sorry!, You cannot save or load Fvaourites!!!")

        except Exception as ex:
            messagebox.showerror("DB Error!", ex)
            self.Button9.configure(state="disabled")    # addFavourite
            self.Button11.configure(state="disabled")   # loadFavourite
            self.Button10.configure(state="disabled")   # removeFavourite
    ...
    def add_song(self):
        ...
        for x in range(0, len(song_names)):
            self.playList.insert(tk.END, song_names[x])
    ...
    def closewindow(self):
        answer = messagebox.askyesno("App Closing!!!", "Do You Want To Quit?")
        if (answer):
    ...
    def load_previous_song(self, e):
        ...
        try:
            ...
            size = self.playList.getsize()
            self.last_index = size - 1

            if self.last_index == 0:
                raise OnlyOneSongError("Only one song present in the list")

            cur_index = self.sel_index_tuple[0]
            if cur_index:
                self.playList.selection_set(cur_index-1)
            else:
                self.playList.selection_set(self.last_index)
            self.playList.selection_clear(cur_index)
            self.play_song()

        except (NoSongSelectedError) as ex1:
            messagebox.showerror("Error!", ex1)

        except (OnlyOneSongError) as ex2:
            messagebox.showerror("Error!", ex2)
    ...
    def load_songs_from_favourites(self):
        ...
        #     messagebox.showerror("DB Error!!!", "Sorry! Songs can't be loaded from Favourites!!!")
    ...
    def load_next_song(self):
        ...
        sel_index_tuple = self.playList.curselection()
        size = self.playList.getsize()
        last_index = size - 1

        cur_index = sel_index_tuple[0]

        if last_index == 0 or cur_index == last_index:
            self.stop_song()
            return
        else:
            self.playList.selection_set(cur_index + 1)
